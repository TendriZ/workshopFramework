...
@php
    ...
    // x_start and y_start are 1-indexed from the controller
    $xStart = isset($x_start) ? max(1, min((int) $x_start, $cols)) : 1;
    $yStart = isset($y_start) ? max(1, min((int) $y_start, $rows)) : 1;

    // Convert to 0-indexed offset
    $startOffset = (($yStart - 1) * $cols) + ($xStart - 1);
                            ...
                            @if ($slot !== null)
                                <div class="label-box">
                                    ...
                                    <div class="barcode-container">
                                        <img src="data:image/svg+xml;base64,{{ $slot['barcode'] }}" style="height: 6px; max-width: 15mm;" alt="barcode">
